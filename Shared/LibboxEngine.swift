...
final class LibboxCoreEngine: CoreEngine, @unchecked Sendable {
    ...
    /// Which inbound shape this engine instance serves; also decides route
    /// management, log verbosity, and debug behavior.
    private let inboundMode: EngineInbound

    init(tunnel: NEPacketTunnelProvider) {
        self.tunnel = tunnel
        self.inboundMode = .tun(autoRoute: true)
    ...
    init(tunFileDescriptor: Int32, autoRoute: Bool = false) {
        self.tunnel = nil
        self.inboundMode = .tun(autoRoute: autoRoute)
        self.platformInterface = WaypoPlatformInterface(tunFileDescriptor: tunFileDescriptor)
    }

    /// System-wide mode construction: no tun device at all — the engine's
    /// inbound is a loopback listener and traffic reaches it through the
    /// system network configuration.
    init(listenerPort: Int) {
        self.tunnel = nil
        self.inboundMode = .mixedListener(port: listenerPort)
        self.platformInterface = WaypoPlatformInterface()
    }

    func start(configuration: TunnelConfiguration, packetFlow: any PacketFlow) async throws {
        let configContent = try EngineConfigBuilder.makeContent(configuration, inbound: inboundMode)
        ...
        switch inboundMode {
        case .tun(let autoRoute): setup.debug = !autoRoute
        case .mixedListener: setup.debug = false
        }
...
final class WaypoPlatformInterface: NSObject, LibboxPlatformInterfaceProtocol, LibboxCommandServerHandlerProtocol {
    ...
    func underNetworkExtension() -> Bool { tunnel != nil }
