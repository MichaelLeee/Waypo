...
- Memory budget for the extension: 50 MB `phys_footprint`, the figure the system terminates a process for. Measured on every commit by the `engine` job's `--footprint` step, which samples before the engine starts, with traffic in flight, and after shutdown, and fails the run past the budget. Baseline for a direct configuration carrying UDP traffic: ~12.7 MiB peak, ~25% of budget, and the peak lands at the closing sample because the footprint climbs for the life of the process and shutdown releases nothing — but on a macOS Release build, which has neither the extension loader nor the device's own limit, so read it as a floor rather than the device figure. Past 60% of budget the step annotates a warning: anything holding per-connection buffers in the extension, starting with the Phase 6 HTTPS decryption work, has to stream with strictly bounded buffers, and past 100% it cannot run in the extension at all. Re-checked per transport added.
